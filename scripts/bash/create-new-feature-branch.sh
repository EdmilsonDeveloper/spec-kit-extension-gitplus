...
while [ $i -le $# ]; do
    ...
    case "$arg" in
        ...
        --help|-h)
            ...
            echo "  --type <type>       Change type spliced into the {type} template token (feature, fix, refactor, ...)"
            ...
            echo ""
            echo "Configuration:"
            echo "  branch_template     Optional gitplus-config.yml template with {author}, {type}, {app}, {number}, {slug}"
...
render_branch_template() {
    ...
    if [ -n "$TYPE_TOKEN" ]; then
        rendered=${rendered//\{type\}/$TYPE_TOKEN}
    else
        rendered=${rendered//\/\{type\}/}
        rendered=${rendered//\{type\}\//}
        rendered=${rendered//\{type\}/}
    fi
...
TYPE_TOKEN=$(clean_branch_name "$TYPE_VALUE")
