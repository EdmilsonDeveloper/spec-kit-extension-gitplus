...
if ($Help) {
    ...
    Write-Host "  -Type <type>        Change type spliced into the {type} template token (feature, fix, refactor, ...)"
    ...
    Write-Host "  branch_template     Optional gitplus-config.yml template with {author}, {type}, {app}, {number}, {slug}"
...
function Expand-BranchTemplate {
    ...
    if ($typeToken) {
        $rendered = $rendered.Replace('{type}', $typeToken)
    } else {
...
$typeToken = ConvertTo-CleanBranchName -Name $Type
